...
| **本步输出** | ① **给“下一步/用户”的报告**：`report/forensics_batch_8.8.8.0_24_1updates_<时间戳>.json` ② **控制台**：该案例的判定结果（most_likely_attacker、命中/未命中）、以及汇总准确率。 |
...
| Step3 | 该目录下 meta + suspicious_updates（+ rag_db） | report/forensics_batch_*.json（含 target、rag_context、chain_of_thought、final_result）+ 控制台准确率 |
...
| **输出** | ① 控制台：每个案例的判定结果、命中/未命中、准确率 ② `report/forensics_batch_*.json`（每事件一份溯源报告） |
...
| 3 溯源评估 | data/events/ | report/forensics_batch_*.json，控制台准确率 |
